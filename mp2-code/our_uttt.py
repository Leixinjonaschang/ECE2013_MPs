...
class ultimateTicTacToe:
    ...
    def checkBlocked(self, isMax, start):
        ...
        for col in range(3):
            ...
            if b[i][col+j] == b[i+2][col+j] and b[i][col+j] == opponent and b[i+1][col+j] == player:
                p += 1
            if b[i+1][col+j] == b[i+2][col] and b[i+1][col+j] == opponent and b[i][col+j] == player:
                p += 1
    ...
    def checkWinner(self):
        ...
        for start in self.globalIdx:
            ...
            #rows
            for row in range(3):
                if b[row][j] == b[row][j+1] and b[row][j] == b[row][j+2]:
                    if b[row][j] == self.maxPlayer:
                        winner = 1
                        return winner
                    elif b[row][j] == self.minPlayer:
                        winner = -1
                        return winner

            #columns        
            for col in range(3):
                if b[i][col] == b[i+1][col] and b[i][col] == b[i+2][col]:
                    if b[i][col] == self.maxPlayer:
                        winner = 1
                        return winner
                    elif b[i][col] == self.minPlayer:
                        winner = -1
                        return winner
                ...
                for i in range(3):
                    for j in range(3):
                        if self.board[start[0]+i][start[1]+j] == '_':                        
                            self.board[start[0]+i][start[1]+j] = self.maxPlayer
                            nextBoard = 3*i+j
                            bestValue = max(bestValue, self.alphabetaDesigned(depth-1, nextBoard, alpha, beta, not isMax))
                            alpha = max(alpha, bestValue)
                            if beta <= alpha:
                                prune = True
                            self.board[start[0]+i][start[1]+j] = '_'
                ...
                for i in range(3):
                    for j in range(3):
                        if self.board[start[0]+i][start[1]+j] == '_':                        
                            self.board[start[0]+i][start[1]+j] = self.minPlayer
                            nextBoard = 3*i+j
                            bestValue = min(bestValue, self.alphabetaDesigned(depth-1, nextBoard, alpha, beta, not isMax))
                            beta = min(beta, bestValue)
                            if beta <= alpha:
                                prune = True
                            self.board[start[0]+i][start[1]+j] = '_'
