...
class Application:
    ...
    def execute(self, filename, searchMethod, save):
        ...
        if not self.__human:
            # path, statesExplored = search(self.maze, searchMethod) # call the search func.
            path, statesExplored, explored_points = search(self.maze, searchMethod)  # call the search func.
        else:
        ...
        if self.__human:
            ...
            self.drawPath(path)
            self.drawExpored(explored_points)
    ...
    # 这个函数用蓝色及其深浅来表征exploration的过程
    def getColorExplored(self, pathLength, index):
        start_color_blue = 0
        end_color_blue = 255
        b_step = (end_color_blue - start_color_blue) / pathLength
        blue = start_color_blue + index * b_step
        return (0,0,blue)
    ...
    # 画出搜索过的点
    def drawExpored(self, explored):
        for p in range(len(explored)):
            color = self.getColorExplored(len(explored),p)
            color = [0.5*i for i in list(color)] # 把颜色 scale 了
            self.drawSquare(explored[p][0],explored[p][1],color)
